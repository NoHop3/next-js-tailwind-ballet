...
import { Event, updateEvent, uploadImage } from '@/lib/supabase';
...
export function EditEventDialog({ event, isOpen, onClose, onSuccess }: EditEventDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let image_url: string | null = imagePreview && !imageFile ? event.image_url : null;

      if (imageFile) {
        const fileName = `${Date.now()}-${imageFile.name.replace(/\s/g, '-')}`;
        const { url, error: uploadError } = await uploadImage(imageFile, fileName);
        if (uploadError) throw new Error(translate('events.edit.imageError'));
        image_url = url;
      }

      const { data, error: updateError } = await updateEvent(event.id, {
        title,
        description,
        link: link || null,
      ...
      if (updateError || !data) throw new Error(translate('events.edit.error'));

      onSuccess(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : translate('events.edit.error'));
    } finally {
      setIsLoading(false);
    }
  };
              ...
              >
                {isLoading ? (
                  <>
                    ...
                    {translate('events.edit.saving')}
                  </>
                ) : (
                  translate('events.edit.save')
                )}
              </Button>
            </div>
          </form>
